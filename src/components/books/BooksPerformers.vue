<script setup>
  ...
  import BooksTitle from '@/components/books/BooksTitle.vue'
...
<template>
  <section class="releases">
    <div class="releases__row">
      ...
      <books-title @click="isRotate = !isRotate" :label="label" :is-rotate="isRotate" />
      <div class="releases__swiper">
        <swiper
          ref="swiperReleases"
          :slides-per-view="8"
          :space-between="20"
          :class="['releases__books', { hidden: isRotate }]"
        >
          <swiper-slide v-for="i in 15" :key="i" @click="checked = !checked" class="releases__books-card">
            <div class="releases__books-image">
              <img src="@/assets/img/performer.png" alt="" />
              <div class="releases__books-play">
                <icon :name="checked ? 'musicPause' : 'musicPlay'" />
              </div>
            </div>
            <div class="releases__books-title">Танцпоп</div>
          </swiper-slide>
        </swiper>
      </div>
